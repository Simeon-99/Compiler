...
//Range
case class Rang(v:Val) extends Val // Range(v:Chr or c:Char)
case class Pls(vs: List[Val]) extends Val
case class Opti(v:Val) extends Val
case class Ntims(vs: List[Val]) extends Val
...
def flatten(v: Val) : String = v match {
  ...
  case Rang(v:Val) => flatten(v)
  case Pls(vs:List[Val]) => vs.map(flatten).mkString
  case Opti(v:Val) => flatten(v)
  case Ntims(vs:List[Val]) => vs.map(flatten).mkString
}
...
def env(v: Val) : List[(String, String)] = v match {
  ...
  case Rang(v:Val) => env(v)
  case Pls(vs:List[Val]) => vs.flatMap(env)
  case Opti(v:Val) => env(v)
  case Ntims(vs:List[Val]) => vs.flatMap(env)
}
...
def mkeps(r: Rexp) : Val = r match {
  ...
  case RANGE(cs:List[Char]) => Rang(Empty)
  case PLUS(r) => Pls(mkeps(r)::Nil)
  case OPTIONAL(r) => if(nullable(r)) Opti(mkeps(r))
                      else Opti(Empty)
  case NTIMES(r,n:Int) => if(nullable(r)) Ntims(mkeps(r)::Nil)
                          else Ntims(Nil)
}
...
def inj(r: Rexp, c: Char, v: Val) : Val = (r, v) match {
  ...
  case(RANGE(cs:List[Char]),Empty) => Rang(Chr(c))
  case(PLUS(r),Sequ(v1,Stars(vs))) => Pls(inj(r,c,v1)::vs)
  case(OPTIONAL(r),_) =>Opti(inj(r,c,v))
  case(NTIMES(r,n),Sequ(v1,Ntims(vs))) => Ntims(inj(r,c,v1)::vs)
}

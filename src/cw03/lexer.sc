...
//|("-" ~(DIGIT | RANGE("123456789".toList)~ DIGIT.%))
...
def main() = {
  ...
  println("============")
  //Ntims
  val reg4 : Rexp = ("ab" | "a") ~ (ONE | "b")
  val reg5 : Rexp = ("aa" | "a").%
  println(lex_simp(reg4,"ab".toList))
  println(lex_simp(reg5,"aaa".toList))
  println("============")
...
def test1(): Unit ={
  val r: Rexp= (ONE | "a").%
  val s = "if33 ifif then then23 else else 32"
  println("start")
  println(mkeps(r))
  println("finish")
